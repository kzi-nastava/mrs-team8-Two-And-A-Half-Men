...
describe('[Student3] Register', () => {
	...
	beforeEach(async () => {
		const unregisteredAuthServiceSpy = jasmine.createSpyObj('UnregisteredAuthService', [
			'register',
		]);
		...
		unregisteredAuthService = TestBed.inject(
			UnregisteredAuthService,
		) as jasmine.SpyObj<UnregisteredAuthService>;
	...
	it('should display validation error for password less than 6 characters', fakeAsync(() => {
		...
		expect(passwordError.nativeElement.textContent).toContain(
	...
	it('should display success popup and navigate to login on successful registration', fakeAsync(() => {
		...
		fixture.detectChanges();
		tick();
		expect(popupsService.success).toHaveBeenCalledWith(
			'Success',
	...
	it('should display error message when server error occurs', fakeAsync(() => {
		component.step = 2;
		fixture.detectChanges();
